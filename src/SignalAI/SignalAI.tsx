...
type Prediction = { symbol:string; generatedAt:string; horizonDays:number; boomDefinition:string; bustDefinition:string; boomProbability:number|null; boomConfidence:number|null; bustProbability:number|null; bustConfidence:number|null; modelVersion:string; boomFactors:{positive:string[];negative:string[]}; bustFactors:{positive:string[];negative:string[]} };
    ...
    <main className="signal-main">
      ...
      <section className="signal-layout">
        ...
        <aside className="signal-insights">
          {prediction?<article className="signal-card prediction-card"><div className="ai-label"><span><Icon name="spark"/></span> VALIDATED {prediction.horizonDays}-DAY OUTLOOK</div><p className="prediction-language">Independent, calibrated historical estimates—not certainty or financial advice.</p><div className="prediction-metrics"><div title={prediction.boomDefinition}><span>Boom chance</span><strong>{prediction.boomProbability==null?"Not validated":`${(prediction.boomProbability*100).toFixed(0)}%`}</strong><small>{prediction.boomConfidence==null?"No reliable confidence rating":`${(prediction.boomConfidence*100).toFixed(0)}% model confidence`}</small></div><div title={prediction.bustDefinition}><span>Bust chance</span><strong>{prediction.bustProbability==null?"Not validated":`${(prediction.bustProbability*100).toFixed(0)}%`}</strong><small>{prediction.bustConfidence==null?"Current model performed near random":`${(prediction.bustConfidence*100).toFixed(0)}% model confidence`}</small></div></div><button className="analysis-button" onClick={()=>setShowAnalysis(!showAnalysis)}>{showAnalysis?"Hide model factors":"Explain these estimates"} <Icon name="arrow"/></button>{showAnalysis&&<div className="expanded-thesis"><b>Boom-supporting factors</b><p>{prediction.boomFactors.positive.join(" · ")||"No strong positive attribution"}</p><b>Bust-risk factors</b><p>{prediction.bustFactors.positive.join(" · ")||"No validated bust attribution"}</p><small>Boom means top-quartile SPY-relative performance; bust means bottom-quartile performance. Model {prediction.modelVersion}</small></div>}</article>:<article className="signal-card prediction-unavailable"><div className="ai-label"><span><Icon name="spark"/></span> PREDICTION ENGINE</div><h2>Validation gate active</h2><p>{predictionStatus}</p><small>Each estimate is published only after it independently passes chronological discrimination and calibration checks.</small></article>}

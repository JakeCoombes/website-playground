...
def fetch_history(symbol: str, api_key: str, cache: Path, refresh: bool) -> pd.DataFrame:
...
def main() -> None:
    parser=argparse.ArgumentParser(); parser.add_argument("--symbols",default="AAPL,MSFT,NVDA,AMZN,GOOGL,META,AVGO,JPM,LLY,TSLA"); parser.add_argument("--benchmark",default="SPY"); parser.add_argument("--output",default="ml/data/training.pkl"); parser.add_argument("--refresh",action="store_true"); parser.add_argument("--max-credits",type=int,default=0,help="Hard upper bound for this run; required with --refresh"); parser.add_argument("--delay",type=float,default=8.0); args=parser.parse_args()
    ...
    required_credits=len(symbols)+1
    if args.refresh and (args.max_credits<=0 or required_credits>args.max_credits):
        raise SystemExit(f"Refresh blocked before any request: needs at most {required_credits} credits; pass --max-credits {required_credits} or a higher explicit limit")
    spy=fetch_history(args.benchmark,key,cache,args.refresh); frames=[]; failures=[]
    for index,symbol in enumerate(symbols):
        try:
            stock=fetch_history(symbol,key,cache,args.refresh); features=engineer_features(stock,spy); targets=create_targets(stock,spy)
            frame=features.join(targets); frame["symbol"]=symbol; frames.append(frame)
        except Exception as error: failures.append(str(error))
        if args.refresh and index<len(symbols)-1: time.sleep(args.delay)
